...
const MortgageCalculator = () => {
    ...
    const getInterestRate = () => {
        ...
        // Корректировака по типу (недвижка)
    ...
    // Для email и текстовых полей сохраняет строку (как она есть)
        if (type === 'email' || type === 'text' || name === 'propertyType' || name === 'loanType') {
            setFormData(prev => ({
                ...prev,
                [name]: value, // email сохраняет как строку (без parseFloat)
            }));
        } else {
    // Для числовых полей
    ...
    // Обработчик слайдеров (вынесен отдельно, для доступности)
