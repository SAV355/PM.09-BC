...
// Формируется текст для шеринга (с проверко наличия платежа)
    const getShareText = () => {
        if (!results) return '';
        const payment = results.monthlyPayment || results.monthlyPension || results.totalMonthlyPayment || 0;
        return `Мой расчёт ${title}: ежемесячный платёж ${formatCurrency(payment)}. Рассчитайте и вы!`;
    };
        ...
        <Container maxWidth="lg">
            <Paper elevation={3} sx={{ p: 4, mt: 4 }}>
                ...
                <Grid container spacing={4}>
                    ...
                    {/* Правая колонка - Результаты и Шеринг */}
                    <Grid item xs={12} md={6}>
                        <Typography variant="h6" gutterBottom sx={{ mb: 3, color: 'secondary.main' }}>
                            Результаты расчета
                        </Typography>
